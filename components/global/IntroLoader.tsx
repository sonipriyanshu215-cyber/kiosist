  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: "linear-gradient(180deg, #030b1a 0%, #050e24 100%)",
        boxShadow: "inset 0 0 0 1px rgba(59,130,246,0.2), inset 0 2px 30px rgba(0,0,0,0.55)",
      }}>
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "9px 18px", gap: 8, background: "rgba(3,9,20,0.92)", borderBottom: "1px solid rgba(59,130,246,0.1)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{ width: 18, height: 18, borderRadius: 5, flexShrink: 0, background: "linear-gradient(135deg, #2563eb, #0891b2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ color: "#fff", fontSize: 10, fontWeight: 900 }}>K</span>
            </div>
          ...
          <svg width="14" height="11" viewBox="0 0 14 11" fill="none" aria-hidden="true">
            <circle cx="7" cy="10" r="1" fill="rgba(255,255,255,0.45)" />
            <path d="M4.8 7.6A3 3 0 0 1 7 6.8a3 3 0 0 1 2.2.8" stroke="rgba(255,255,255,0.45)" strokeWidth="1.3" strokeLinecap="round" />
            <path d="M2.6 5.2A6.2 6.2 0 0 1 7 3.4a6.2 6.2 0 0 1 4.4 1.8" stroke="rgba(255,255,255,0.35)" strokeWidth="1.3" strokeLinecap="round" />
          </svg>
          <div style={{ display: "flex", alignItems: "center", gap: 1 }}>
            <div style={{ width: 20, height: 10, border: "1px solid rgba(255,255,255,0.28)", borderRadius: 2.5, padding: "1.5px", display: "flex" }}>
              <div style={{ width: "80%", background: "#34d399", borderRadius: 1 }} />
            </div>
            <div style={{ width: 2, height: 5, background: "rgba(255,255,255,0.28)", borderRadius: 1 }} />
          </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", background: "rgba(3,10,22,0.75)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ width: 44, height: 44, borderRadius: 13, flexShrink: 0, background: "linear-gradient(135deg, #1d4ed8, #0891b2)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 14px rgba(29,78,216,0.5)" }}>
            <span style={{ color: "#fff", fontWeight: 900, fontSize: 18, fontFamily: "var(--font-display),sans-serif" }}>K</span>
          </div>
          <div>
        ...
        <div style={{ padding: "20px 18px 14px" }}>
          <div style={{ display: "flex", gap: 11, alignItems: "flex-start" }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, flexShrink: 0, background: "rgba(29,78,216,0.3)", border: "1px solid rgba(59,130,246,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <span style={{ color: "#93c5fd", fontSize: 13, fontWeight: 800 }}>K</span>
            </div>
                  ...
                  {sub === "typing" ? (
                    ...
                    <motion.div key="msg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.18 }}>
                      {MSG_LINES.slice(0, lines).map((line, i) => (
                        <motion.p key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.38, ease: [0.16, 1, 0.3, 1] }} style={{ margin: 0, marginBottom: i < lines - 1 ? 6 : 0, fontSize: 14, lineHeight: 1.68, color: i === 1 ? "#93c5fd" : "#d1d9f0", fontWeight: i === 1 ? 600 : 400, fontFamily: "var(--font-body),sans-serif" }}>
                          {line}
                        </motion.p>
                      ))}
                    </motion.div>
          ...
          <AnimatePresence>
            {sub === "cta" && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}>
                <motion.button
                  onClick={onEnter}
                  whileHover={rm ? {} : { scale: 1.02, y: -2, boxShadow: "0 16px 44px rgba(59,130,246,0.55)" }}
                  whileTap={rm ? {} : { scale: 0.97 }}
                  style={{ width: "100%", padding: "14px 24px", borderRadius: 99, border: "none", background: "linear-gradient(135deg, #3b82f6, #06b6d4)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: "0 8px 28px rgba(59,130,246,0.4)", fontFamily: "var(--font-body),sans-serif" }}
                >
                  Explore Kiosist
                ...
                <button
                  onClick={onEnter}
                  style={{ marginTop: 10, width: "100%", background: "none", border: "none", color: "rgba(143,163,196,0.38)", fontSize: 12, cursor: "pointer", padding: "4px 0", fontFamily: "var(--font-body),sans-serif", transition: "color 0.2s" }}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "rgba(143,163,196,0.75)"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "rgba(143,163,196,0.38)"; }}
                >
                  Skip intro
                </button>
              </motion.div>
...
/* ── Wave background ────────────────────────────────────────────── */
type WaveDef = { y: number; a: number; f: number; s: number; p: number; rgb: [number, number, number]; op: number; w: number };

const WAVE_DEFS: WaveDef[] = [
  { y: 0.10, a: 20, f: 0.0095, s: 0.22, p: 0.00, rgb: [59,  130, 246], op: 0.07, w: 1.0 },
  { y: 0.22, a: 32, f: 0.0070, s: 0.17, p: 1.20, rgb: [99,  102, 241], op: 0.09, w: 1.5 },
  { y: 0.34, a: 46, f: 0.0058, s: 0.24, p: 2.40, rgb: [59,  130, 246], op: 0.11, w: 2.0 },
  { y: 0.45, a: 58, f: 0.0048, s: 0.14, p: 0.80, rgb: [6,   182, 212], op: 0.13, w: 2.5 },
  { y: 0.55, a: 58, f: 0.0048, s: 0.20, p: 3.10, rgb: [59,  130, 246], op: 0.13, w: 2.5 },
  { y: 0.66, a: 46, f: 0.0058, s: 0.21, p: 1.60, rgb: [6,   182, 212], op: 0.10, w: 2.0 },
  { y: 0.78, a: 32, f: 0.0075, s: 0.27, p: 2.00, rgb: [99,  102, 241], op: 0.08, w: 1.5 },
  { y: 0.90, a: 20, f: 0.0095, s: 0.30, p: 0.50, rgb: [59,  130, 246], op: 0.06, w: 1.0 },
];
...
function WaveCanvas() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      WAVE_DEFS.forEach(({ y, a, f, s, p, rgb, op, w }) => {
        const [r, g, b] = rgb;
        const baseY = canvas.height * y;

        const path = new Path2D();
        for (let x = 0; x <= canvas.width; x += 3) {
          const py = baseY + Math.sin(x * f + t * s + p) * a;
          x === 0 ? path.moveTo(x, py) : path.lineTo(x, py);
        }

        /* Glow pass */
        ctx.save();
        ctx.strokeStyle = `rgba(${r},${g},${b},${(op * 0.55).toFixed(3)})`;
        ctx.lineWidth   = w * 6;
        ctx.shadowBlur  = 18;
        ...
        /* Core line */
        ctx.save();
        ctx.strokeStyle = `rgba(${r},${g},${b},${(op * 1.5).toFixed(3)})`;
        ctx.lineWidth   = w;
        ctx.stroke(path);
        ctx.restore();
      });
...
/* ── Beam fan config — focused toward screen panel (≈+25° center) ── */
const BEAMS = [
...
export function IntroLoader() {
  ...
  const kioskControls = useAnimation();
  ...
  useEffect(() => {
    ...
    kioskControls.start({
      ...
      x:       0,
      opacity: [0, 1, 1],
      transition: { duration: 2.0, times: [0, 0.86, 1], ease: "easeOut" },
    });
  }, [show, rm, kioskControls]);

  useEffect(() => { if (rm && show) setPhase("message"); }, [rm, show]);
  ...
  }, [phase, rm, kioskControls]);

  const handleEnter = useCallback(() => setShow(false), []);
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 10010 }}>
          <div style={{ perspective: "700px", perspectiveOrigin: "50% 50%" }}>
            <motion.div
              animate={kioskControls}
              initial={{ rotateY: -48, rotateX: 8, scale: 0.82, y: 48, x: 0, opacity: 0 }}
            ...
            >
              ...
              <AnimatePresence>
                {(phase === "glow" || phase === "zoom" || phase === "message") && (
                  <motion.div key="sg"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: phase === "message" ? 0.55 : [0, 1, 0.7] }}
                    exit={{ opacity: 0 }}
                    transition={{ duration: 0.9 }}
                    style={{ position: "absolute", inset: 0, borderRadius: 14, background: "radial-gradient(ellipse 68% 38% at 50% 32%,rgba(59,130,246,0.6) 0%,rgba(6,182,212,0.2) 44%,transparent 68%)", pointerEvents: "none" }}
                  />
                )}
              </AnimatePresence>
        ...
        {/* ── Pulse rings (origin = kiosk screen area) ── */}
        <AnimatePresence>
          {screenActive && !rm && [0, 1, 2].map((i) => (
            <motion.div
              ...
              animate={{ width: [50, 380], height: [50, 380], opacity: [0.65, 0] }}
              transition={{ duration: 2.4, delay: i * 0.82, repeat: Infinity, ease: "easeOut" }}
            />
          ))}
        </AnimatePresence>

        {/* ── Ray beams from kiosk screen ── */}
        <AnimatePresence>
          {screenActive && !rm && (
            <motion.div
              key="beams"
              ...
              style={{ position: "fixed", left: "calc(50% - 22vw)", top: "calc(50% - 319px)", zIndex: 10056, pointerEvents: "none" }}
            >
              {BEAMS.map((b, i) => (
                <motion.div
                  key={i}
                  style={{
                    ...
                    height: b.w,
                    background: `linear-gradient(90deg, rgba(59,130,246,${b.op + 0.1}), rgba(6,182,212,${b.op * 0.45}), transparent)`,
                    transform: `rotate(${b.a}deg)`,
                    transformOrigin: "0% 50%",
                    borderRadius: 99,
                  }}
                  animate={{ opacity: [0, b.op, b.op * 0.6, b.op] }}
                  transition={{ duration: 2.3, delay: b.delay + 0.4, repeat: Infinity, ease: "easeInOut" }}
                />
              ))}
            </motion.div>
          )}
        ...
        {/* ── Screen panel — right side ── */}
        <AnimatePresence>
          {screenActive && (
            <motion.div
              key="screen"
              style={{ position: "fixed", inset: 0, display: "flex", alignItems: "center", justifyContent: "flex-end", paddingRight: "clamp(16px, 5vw, 72px)", zIndex: 10060, pointerEvents: "none" }}
              exit={{ opacity: 0, transition: { duration: 0.3 } }}
            >
              <motion.div
                style={{ pointerEvents: "auto" }}
                initial={{ scale: 0.10, opacity: 0 }}
                animate={phase === "zoom" ? { scale: [0.10, 0.065, 1], opacity: [0, 1, 1] } : { scale: 1, opacity: 1 }}
                exit={{ scale: 0.07, opacity: 0 }}
                transition={{ scale: { duration: 1.05, times: [0, 0.27, 1], ease: "easeInOut" }, opacity: { duration: 0.20 } }}
              >
